...
server = function(input, output, session) {
  ...
  observeEvent(c(input$selectInput, input$csv), {
    if (input$selectInput == "usedemo") {
      x <- readr::read_csv("www/demo_segments.csv")
    } else if (input$selectInput == "usegpkg") {
      req(input$csv)
      x <- readr::read_csv(input$csv$datapath)
    }
    seasons <- x$season
  ...
  gps_csv <- eventReactive(input$selectInput, {
    if (input$selectInput == "usedemo") {
      ...
      req(input$gpkg)
      readr::read_csv(input$gpkg$datapath) |>
        mutate(year=year(time), yday=yday(time))
    }
  })
